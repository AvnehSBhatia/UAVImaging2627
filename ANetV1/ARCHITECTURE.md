...
- **Fast local inference:** `scripts/export_onnx.py --ckpt runs/anet/best.pt` produces a self-contained ONNX (sidecar weights re-inlined — the ORT CoreML partitioner can't read `.onnx.data`); `anet.onnxrt.OnnxANet` runs it. Measured (M-series, batch 1, hidden=24): ~36 ms/img CoreML-GPU vs ~92 ms eager MPS vs ~436 ms eager CPU, exact argmax parity. Eager PyTorch is launch-bound (~89% of time in the Stage-1 small-kernel cascade); ORT/CoreML fuses it. `MLComputeUnits: ALL` (GPU); ANE-only is 4× slower from per-partition hops.
