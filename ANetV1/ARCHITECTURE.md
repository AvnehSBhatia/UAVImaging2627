...
**D53 — dense-path speed (guaranteed, independent of the Triton kernel).** (1) `MIOPEN_FIND_MODE` FAST→NORMAL: FAST never retries when a conv's workspace request is denied → permanent slow fallback (the `provided ptr: 0` log spam); NORMAL does one cached timed search per static shape. (2) `EdgeDQStem4`'s 4 separate `groups=3` depthwise 7×7 convs (88.6% of grouped-conv FLOPs, run at full 540×960) fused into one `groups=12` conv — bit-identical (max Δ 0.0), one MIOpen dispatch on a less pathological shape. (3) `samples_per_epoch` was uncapped on ROCm (full ~13.5k → ~60 min/epoch); `ANET_SAMPLES=6000` gives eval/checkpoint feedback ~2.3× faster (pure speed lever — the sampler redraws i.i.d. from a fixed distribution). (4) A fused *backward* kernel crash now demotes only the backward to chunked-autograd (keeping the fast fused forward and the large batch) instead of collapsing to dense/batch-32, with full-traceback diagnostics.
